...
function stability_check_with_slider()
    ...
    % Create the figure
    fig = figure('Name', 'Stability Check with Slider', 'NumberTitle', 'off', 'Position', [100, 100, 500, 350]);

    % Create a text box to display stability message
    textBox = uicontrol('Style', 'text', 'Position', [50, 300, 400, 30], 'HorizontalAlignment', 'center', ...
        'FontWeight', 'bold', 'FontSize', 12);

    % Create the slider
    slider = uicontrol('Style', 'slider', 'Min', 0, 'Max', 50, 'Value', 5, ...
        'Position', [100, 250, 300, 20], 'Callback', @update_stability);

    % Create a text label to show the slider value
    sliderLabel = uicontrol('Style', 'text', 'Position', [200, 230, 100, 20], ...
        'String', 'K = 5');
